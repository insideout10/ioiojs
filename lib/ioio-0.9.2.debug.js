  ...
  (function($) {
    return $.fn.arrowscrollers = function(options) {
      ...
      return this.each(function() {
        ...
        arrowRight = $("<div class=\"" + settings.classNames.arrowscroller + " " + settings.classNames.arrow + " " + settings.classNames.right + "\"></div>").css('width', settings.arrow.width).css('height', height).css('margin-left', width + margin).css('cursor', 'pointer').click(doScrollLeft);
        element.css('margin-left', margin).before(arrowLeft).after(arrowRight);
        if (true === setttings.marginTop) {
